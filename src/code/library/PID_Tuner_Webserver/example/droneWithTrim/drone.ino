...
float kp_rate_yaw = 0, ki_rate_yaw = 0, kd_rate_yaw = 0;
...
double kp_angle_roll = 0, ki_angle_roll = 0, kd_angle_roll = 0;
...
void motorControl() {
  ...
  // Standard X config (M1 FR, M2 FL, M3 RL, M4 RR)

// pwm_1 = constrain(throttle + pwmOut_pitch + pwmOut_roll + pwmOut_yaw + u3, 0, batas); // M1 - rear-left
// pwm_2 = constrain(throttle + pwmOut_pitch - pwmOut_roll - pwmOut_yaw + u4, 0, batas); // M2 - rear-right
// pwm_3 = constrain(throttle - pwmOut_pitch - pwmOut_roll + pwmOut_yaw + u1, 0, batas); // M3 - front-right
// pwm_4 = constrain(throttle - pwmOut_pitch + pwmOut_roll - pwmOut_yaw + u2, 0, batas); // M4 - front-left

pwm_1 = constrain(throttle + pwmOut_pitch + pwmOut_roll - pwmOut_yaw + u1 + tuner.getTrimM1(), 0, batas); // M1
pwm_2 = constrain(throttle + pwmOut_pitch - pwmOut_roll + pwmOut_yaw + u2 + tuner.getTrimM2(), 0, batas); // M2
pwm_3 = constrain(throttle - pwmOut_pitch - pwmOut_roll - pwmOut_yaw + u3 + tuner.getTrimM3(), 0, batas); // M3
pwm_4 = constrain(throttle - pwmOut_pitch + pwmOut_roll + pwmOut_yaw + u4 + tuner.getTrimM4(), 0, batas); // M4
...
void motorTest() {
  ledcWrite(1, 50);  // Motor kanan (Right)
  // ledcWrite(2, 70);  // Motor kiri (Left)
}
